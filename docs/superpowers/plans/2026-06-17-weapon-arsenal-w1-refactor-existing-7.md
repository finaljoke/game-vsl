...
- [ ] **Step 1: 更新 `tests/test_weapons_new.gd` 的 whip 断言（先失败）**
...
- [ ] **Step 3: 重写 `data/weapons/whip.tres`**
...
- [ ] **Step 4: 改 `autoloads/card_pool.gd` 的 whip 卡文案**
...
- [ ] **Step 1: 新建 `tests/test_weapons_w1.gd` 并写回旋斧反射断言（先失败）**
...
- [ ] **Step 3: 重写 `data/weapons/boomerang.tres`**
...
- [ ] **Step 4: 改 `card_pool.gd` 的 boomerang 卡文案**
...
- [ ] **Step 5: 运行，确认通过** — Run: `test_weapons_w1` → PASS。
...
- [ ] **Step 1: 向 `tests/test_weapons_w1.gd` 追加反射断言（先失败）**
...
- [ ] **Step 3: 改 `scenes/weapons/orb/orb_shield.gd`（常量→可注入 var）**
...
- [ ] **Step 4: 改 `scenes/weapons/orb/orb_weapon.gd`（声明字段 + 注入到护盾球）**
...
- [ ] **Step 5: 重写 `data/weapons/orb.tres`**
...
- [ ] **Step 6: 改 `card_pool.gd` 的 orb 卡文案**
...
- [ ] **Step 1: 向 `tests/test_weapons_w1.gd` 追加 burn 附着断言（先失败）**
...
- [ ] **Step 3: 改 `scenes/weapons/aura/aura_weapon.gd`**
...
- [ ] **Step 4: 重写 `data/weapons/aura.tres`**
...
- [ ] **Step 5: 改 `card_pool.gd` 的 aura 卡文案**
...
- [ ] **Step 1: 向 `tests/test_weapons_w1.gd` 追加 stun 断言（先失败）**
...
- [ ] **Step 4: 重写 `data/weapons/lightning.tres`**
...
- [ ] **Step 5: 改 `card_pool.gd` 的 lightning 卡文案**
...
- [ ] **Step 1: 向 `tests/test_weapons_w1.gd` 追加暴击纯函数 + 反射断言（先失败）**
...
- [ ] **Step 4: 改 `scenes/weapons/knife/knife_projectile.gd`（弹速可注入）**
...
- [ ] **Step 5: 重写 `data/weapons/knife.tres`**
...
- [ ] **Step 6: 更新 `tests/test_card_pool.gd` 的 knife Lv3 冷却断言**
...
- [ ] **Step 7: 改 `card_pool.gd` 的 knife 卡文案**
...
- [ ] **Step 1: 新建 `tests/test_burn_field.gd`（先失败）**
...
- [ ] **Step 3: 创建 `scenes/weapons/explosion/burn_field.gd`**
...
- [ ] **Step 4: 运行 `test_burn_field`，确认通过** — Run: `test_burn_field` → PASS。

- [ ] **Step 5: 改 `scenes/weapons/explosion/explosion.gd`（blast_radius 可注入）**
...
- [ ] **Step 6: 改 `scenes/weapons/explosion/explosion_weapon.gd`（声明字段 + 注入半径 + 生成地火）**
...
- [ ] **Step 7: 重写 `data/weapons/explosion.tres`**
...
- [ ] **Step 8: 向 `tests/test_weapons_w1.gd` 追加火球反射断言**
...
- [ ] **Step 9: 改 `card_pool.gd` 的 explosion 卡文案**
