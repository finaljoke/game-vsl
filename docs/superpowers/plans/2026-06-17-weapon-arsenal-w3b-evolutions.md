...
- [x] **Step 1: 新建 `tests/test_weapons_w3b.gd` 并写 Whirlwind 测试（先失败）**
...
- [x] **Step 2: 运行，确认失败** — Run: `<TEST_FILE>` = `test_weapons_w3b` → 红（`full_circle` 字段不存在）。

- [x] **Step 3: 改 `scenes/weapons/whip/whip_weapon.gd`**
...
- [x] **Step 2: 运行，确认失败** — `test_weapons_w3b` 红（`volley` 字段不存在）。
...
- [x] **Step 4: 重写 `data/weapons/thousand_edge.tres` 的 levels**（保留 cooldown 0.15/pierce 8/proj_scale 1.7/proj_tint）
...
- [x] **Step 5: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；并跑 `test_card_pool`（`test_apply_evolve_knife` cooldown 0.15/pierce 8 不回归）。
...
- [x] **Step 2: 运行，确认失败** — 红（`orbit_return` 不存在）。

- [x] **Step 3: 改 `scenes/weapons/boomerang/boomerang_projectile.gd`**
...
- [x] **Step 4: 改 `scenes/weapons/boomerang/boomerang_weapon.gd`**

字段区加 `var orbit_return: bool = false`；`attack()` 内 `proj.max_range = throw_range` 之后加 `proj.orbit_return = orbit_return`。

- [x] **Step 5: 重写 `data/weapons/cyclone.tres` 的 levels**（保留 count 3/pierce 8/throw_range 300）

```
levels = [{"cooldown": 0.7, "pierce": 8, "throw_range": 300.0, "count": 3, "orbit_return": true, "damage": 20.0}]
```

- [x] **Step 6: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → Cyclone PASS。
...
- [x] **Step 2: 运行，确认失败** — 红（`secondary_count` 不存在）。

- [x] **Step 3: 改 `scenes/weapons/explosion/explosion_weapon.gd`**
...
- [x] **Step 4: 重写 `data/weapons/nuke.tres` 的 levels**（保留 cooldown 0.5/blast_scale 1.6/blast_tint）
...
- [x] **Step 5: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；`test_card_pool`（nuke cooldown 0.5/blast_scale>1 不回归）。
...
- [x] **Step 2: 运行，确认失败** — 红（inferno_aura `burn_dps` 反射为默认 0）。

- [x] **Step 3: 重写 `data/weapons/inferno_aura.tres` 的 levels**（加 burn_dps，保留 radius 170/lifesteal 0.3/cooldown 0.4）

```
levels = [{"cooldown": 0.4, "radius": 170.0, "lifesteal_on_hit": 0.3, "burn_dps": 10.0, "damage": 12.0}]
```

- [x] **Step 4: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；`test_weapons_new`（`test_evolve_aura` radius 170 不回归）。
...
- [x] **Step 2: 运行，确认失败** — 红（`sky_strikes` / `_sky_strike` 不存在）。
...
- [x] **Step 4: 重写 `data/weapons/thunderstorm.tres` 的 levels**（保留 chains 8/bolt_tint/cooldown 0.45）
...
- [x] **Step 5: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；`test_weapons_new`（`test_evolve_lightning` chains 8 / `test_thunderstorm_bolt_tint` 不回归）。
...
- [x] **Step 2: 运行，确认失败** — 红（`dash_enabled` 不存在）。

- [x] **Step 3: 改 `scenes/weapons/orb/orb_shield.gd`**
...
- [x] **Step 4: 改 `scenes/weapons/orb/orb_weapon.gd`**
...
- [x] **Step 5: 重写 `data/weapons/mega_orb.tres` 的 levels**（保留 total_orbs 8）

```
levels = [{"total_orbs": 8, "damage": 8.0, "dash_enabled": true, "dash_interval": 3.0}]
```

- [x] **Step 6: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；`test_card_pool`（`test_apply_evolve_orb` total_orbs 8 不回归）。
...
- [x] **Step 1: （若 W3a 未做）把 `tests/test_weapons_new.gd` 的 `test_all_evolvable_count_is_seven` 改为集合成员断言**
...
- [x] **Step 2: 向 `test_weapons_w3b.gd` 追加 Earthshatter 测试（先失败）**
...
- [x] **Step 3: 运行，确认失败** — 红（`earthshatter missing` / `_apply_shockwave` 不存在）。

- [x] **Step 4: 改 `scenes/weapons/maul/maul_weapon.gd`**
...
- [x] **Step 5: 给 `data/weapons/maul.tres` 加 evolution**

```
evolution = {"requires_perk": "perk_hp", "requires_perk_stacks": 3, "evolved_id": "earthshatter"}
```

- [x] **Step 6: 创建 `data/weapons/earthshatter.tres`**
...
- [x] **Step 7: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS；`test_weapons_new`（集合成员断言 PASS）。
...
- [x] **Step 2: 运行，确认失败** — 红（`Could not preload .../snow_field.gd`）。

- [x] **Step 3: 创建 `scenes/weapons/frostbite/snow_field.gd`**
...
- [x] **Step 4: 改 `scenes/weapons/frostbite/frostbite_weapon.gd`**
...
- [x] **Step 5: 给 `data/weapons/frostbite.tres` 加 evolution**

```
evolution = {"requires_perk": "perk_attack", "requires_perk_stacks": 3, "evolved_id": "blizzard"}
```

- [x] **Step 6: 创建 `data/weapons/blizzard.tres`**
...
- [x] **Step 7: 刷新缓存 + 测试** — `--import`，`test_weapons_w3b` → PASS。
...
- [x] **Step 2: 运行，确认失败** — 红（`singularity missing` / `collapse_damage` 不存在）。

- [x] **Step 3: 改 `scenes/weapons/gravity_well/gravity_well.gd`**
