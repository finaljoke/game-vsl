...
- [ ] **Step 1: 新建 `tests/test_weapons_w2.gd` 并写 Maul 测试（先失败）**
...
- [ ] **Step 3: 创建 `scenes/weapons/maul/maul_weapon.gd`**
...
- [ ] **Step 4: 创建 `scenes/weapons/maul/maul_weapon.tscn`**
...
- [ ] **Step 5: 创建 `data/weapons/maul.tres`**
...
- [ ] **Step 6: 在 `autoloads/card_pool.gd` 注册 maul**
...
- [ ] **Step 8: 运行测试，确认通过** — Run: `<TEST_FILE>` = `test_weapons_w2` → Maul 4 个测试 PASS。
...
- [ ] **Step 1: 向 `tests/test_weapons_w2.gd` 追加 Frostbite 测试（先失败）**
...
- [ ] **Step 2: 运行，确认失败** — Run: `test_weapons_w2` → 红（`frostbite missing in WeaponDB`）。

- [ ] **Step 3: 创建 `scenes/weapons/frostbite/frostbite_weapon.gd`**
...
- [ ] **Step 4: 创建 `scenes/weapons/frostbite/frostbite_weapon.tscn`**
...
- [ ] **Step 5: 创建 `data/weapons/frostbite.tres`**
...
- [ ] **Step 6: 在 `autoloads/card_pool.gd` 注册 frostbite**
...
- [ ] **Step 7: 刷新导入/类缓存** — Run: `--import`。Expected: 无错误/无未知字段告警。

- [ ] **Step 8: 运行测试，确认通过** — Run: `test_weapons_w2` → Frostbite 测试 PASS（Maul 不回归）。
...
- [ ] **Step 1: 向 `tests/test_weapons_w2.gd` 追加引力井测试（先失败）**
...
- [ ] **Step 2: 运行，确认失败** — Run: `test_weapons_w2` → 红（`Could not preload .../gravity_well.gd`）。

- [ ] **Step 3: 创建场实体 `scenes/weapons/gravity_well/gravity_well.gd`**
...
- [ ] **Step 6: 创建 `scenes/weapons/gravity_well/gravity_well_weapon.tscn`**
...
- [ ] **Step 7: 创建 `data/weapons/gravity_well.tres`**
...
- [ ] **Step 8: 在 `autoloads/card_pool.gd` 注册 gravity_well**
...
- [ ] **Step 9: 刷新导入/类缓存** — Run: `--import`。Expected: 无错误/无未知字段告警。

- [ ] **Step 10: 运行测试，确认通过** — Run: `test_weapons_w2` → 全部 PASS（Maul/Frostbite 不回归）。
