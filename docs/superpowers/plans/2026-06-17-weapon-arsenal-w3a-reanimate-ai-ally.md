...
- [ ] **Step 1: 新建 `tests/test_reanimate.gd` 并写随从测试（先失败）**
...
- [ ] **Step 2: 运行，确认失败** — Run: `<TEST_FILE>` = `test_reanimate` → 红（`Could not preload .../roaming_minion.gd`）。

- [ ] **Step 3: 创建 `scenes/weapons/reanimate/roaming_minion.gd`**
...
- [ ] **Step 4: 运行，确认通过** — Run: `test_reanimate` → 5 个随从测试 PASS。
...
- [ ] **Step 1: 向 `tests/test_reanimate.gd` 追加武器测试（先失败）**
...
- [ ] **Step 2: 运行，确认失败** — Run: `test_reanimate` → 红（`reanimate missing in WeaponDB`）。

- [ ] **Step 3: 创建 `scenes/weapons/reanimate/reanimate_weapon.gd`**
...
- [ ] **Step 4: 创建 `scenes/weapons/reanimate/reanimate_weapon.tscn`**
...
- [ ] **Step 5: 创建 `data/weapons/reanimate.tres`**（W3a 暂 `evolution={}`，Task 3 补）
...
- [ ] **Step 6: 在 `autoloads/card_pool.gd` 注册 reanimate**
...
- [ ] **Step 7: 刷新导入/类缓存** — Run: `--import`。Expected: 无 `SCRIPT ERROR`/`Parse Error`；`reanimate.tres` 无未知字段告警。

- [ ] **Step 8: 运行测试，确认通过** — Run: `test_reanimate` → 武器测试 PASS（随从测试不回归）。
...
- [ ] **Step 1: 更新 `tests/test_weapons_new.gd` 的 evolvable 断言 + 向 `test_reanimate.gd` 加进化测试（先失败）**
...
- [ ] **Step 3: 给 `data/weapons/reanimate.tres` 加 evolution**

```
evolution = {"requires_perk": "perk_hp", "requires_perk_stacks": 3, "evolved_id": "horde"}
```

- [ ] **Step 4: 创建 `data/weapons/horde.tres`**
...
- [ ] **Step 5: 刷新导入/类缓存** — Run: `--import`。Expected: 无错误；`horde.tres`、`reanimate.tres` 无未知字段告警。
