...
- [ ] **Step 1: 写失败测试 `tests/test_status_component.gd`**
...
- [ ] **Step 3: 创建实现 `scenes/enemies/status_component.gd`**
...
- [ ] **Step 1: 写失败测试 `tests/test_enemy_status.gd`**
...
- [ ] **Step 3: 修改 `scenes/enemies/enemy.gd`**
...
- [ ] **Step 1: 写失败测试 `tests/test_enemy_velocity.gd`（纯静态合成）**
...
- [ ] **Step 3: 修改 `scenes/enemies/enemy.gd`（外力 + 合成）**
...
- [ ] **Step 4: 向 `tests/test_enemy_status.gd` 追加击退集成测试**
...
- [ ] **Step 1: 向 `tests/test_player.gd` 追加 `sum_contact_damage` 失败测试**
...
- [ ] **Step 3: 修改 `scenes/player/player.gd`（抽纯函数 + 跳过硬直）**
...
- [ ] **Step 5: 向 `tests/test_enemy_status.gd` 追加移动门控集成测试**
...
- [ ] **Step 1: 写失败测试 `tests/test_weapon_crit.gd`**
...
- [ ] **Step 3: 修改 `scenes/player/player.gd`（新增 crit 字段）**
...
- [ ] **Step 4: 修改 `scenes/weapons/weapon_base.gd`（damage_for 重载 + crit_multiplier）**
...
- [ ] **Step 5: 向 `tests/test_player.gd` 追加 crit 默认值测试**
...
- [ ] **Step 2: 资源导入 + 解析检查（确认新脚本无导入/解析错误）**
...
- [ ] **Step 4: 不提交（纯验证）。** 若任一步红，回到对应 Task 修复并重跑。
