...
- [ ] **Step 1: 写失败测试（`tests/test_vfx_weapon_fx.gd`，含 helper）**
...
- [ ] **Step 3: 改 `whip_weapon.gd` 的 `attack()`**
...
- [ ] **Step 1: 写失败测试（追加到 `tests/test_vfx_weapon_fx.gd`）**
...
- [ ] **Step 3a: 改 `knife_projectile.gd` 命中处**
...
- [ ] **Step 3b: 改 `knife_weapon.gd` 的 `attack()`**
...
- [ ] **Step 3: 改 `boomerang_projectile.gd` 的 `_ready()`**
...
- [ ] **Step 3a: 改 `explosion.gd` 的 `detonate()`**
...
- [ ] **Step 3b（可选清理）：** 既有单张贴图 `_process` 缩放淡出可保留作底层闪光，或在确认序列帧观感后由视觉冒烟决定是否弱化其 alpha。本步不强制改 `_process`。
...
- [ ] **Step 3: 改 `aura_weapon.gd` 的 `_setup_ring()`**
...
- [ ] **Step 2: 视觉冒烟（godot-ai MCP，逐武器目视，非阻塞）**
