...
2. **C5 确定性**:`--bot=kite --cards=default --fast=3 --maxtime=600` seed 7 两跑 `summary+events+tick` 逐字节一致,**至少到 t≈560s 或满程胜利**;仅容许 [[project-vsl-bot-telemetry]] 已记录的"极密终幕(300 敌+Boss)引擎级浮点微抖"(对 A/B 无影响)。
