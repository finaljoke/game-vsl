...
P2b §4 地板档证明 thousand_edge(knife)/mega_orb(orb) 纯 solo + perk_hp×5 仍够不到稳定进化窗口（base 太脆）。P3 在「有保命武器」的混编 build 里测，**A/B 边际归因**隔离目标进化贡献：

- **底盘（chassis）** = `frostbite`（控制/slow，低清场）+ 开局 perk_hp×5（+100 HP 防御垫）。两约束实测达成：① **可达**——mix_knife/mix_orb 烟测（seed 7）均触发进化（t_evo 213.5/200.0）；② **留 headroom**——mixbase 后期 `enemies_alive` 维持 33–47（底盘不自清场，留敌供目标清）。
- **A/B**：`mix_<target>` vs `mixbase`（同种子），在目标进化窗 `[t_evo, min(end_mix, end_base)]` **共时重叠窗**内比 backlog；`marginal = backlog_base − backlog_mix`（正 = 目标降低积压 = 清场贡献）。底盘早死无碍（取重叠窗右端）。
- **控制组 `explosion`**（solo 已知最强清场 backlog 5）：给 knife/orb 的边际清场定标——knife 边际 ≈ explosion ⇒ thousand_edge 亦强清场（OP 嫌疑）；远低 ⇒ 假说证伪。
- **标定中发现并修复的 harness bug**：底盘 frostbite 与 knife 共享 `perk_attack` 进化门，满级 + perk_attack×3 后 frostbite 也就绪；确定性进化投放按 weapon-id 字典序取首（`frostbite`<`knife`），bot 只消费目标进化 → 永久挡死 `evolve_knife`（实测 knife_3+perk_attack×6 仍不进化）。修：`_grant_mix` banish `evolve_<chassis>`，被 `ready_evolutions` 的 banish 守卫滤除（commit 1bab639）。

### 3b. A/B 结果与终判

主 campaign：mixbase + mix_{knife,orb,explosion} × 8 种子（7/42/101/1-5），`--fixed-fps 60 --fast=8 --maxtime=600`，32 run。

| 目标 | reached | backlog_mix | backlog_base | **marginal** | clear_eff(ctx) | hp_min | 终判 |
|---|---|---|---|---|---|---|---|
| **explosion**（控制） | **8/8** | 4 | 29 | **+25** | 41.9 | 0.99 | 强清场基准（nuke，§1d 已 OP） |
| **knife**（thousand_edge） | **8/8** | 14 | 30 | **+16** | 15.7 | 0.88 | **强清场 + 安全**（≈ nuke 64%）→ OP 嫌疑 |
| **orb**（mega_orb） | **8/8** | 27 | 29 | **+3** | 6.3 | 0.18 | **清场可忽略 + 不安全** → 非 OP，疑偏弱 |

**① 可达性结论（颠覆 P2a/P2b）**：thousand_edge/mega_orb 在混编里 **reached 8/8**（纯 solo 0/0、地板档 0.25/0.13）。**确证 P2b §4 假说：二者 solo「不可达」是 base 武器太脆，非进化弱**。给它们保命底盘活到进化，进化窗稳定可测。「别据 solo reached=0 判进化弱」（spec §4b）实证闭环。

**② thousand_edge（knife）终判：强清场 + 安全，OP 嫌疑 → P3b**。边际 +16（把 backlog 30→14），达控制组 nuke（+25）的 64%，且安全（hp_min 0.88）。**「绕冷却 OP」假说部分支持**——它确是强清场者（远超 orb 的 +3），虽不及 nuke 极端。结合 §1d 的 nuke/thunderstorm/earthshatter，「强清场 + 安全」是一致的 OP 签名。**本轮不调数值**（测量+终判），记 **P3b 优先复核**。注：P2a/P2b 的「weak†/不可达」对 thousand_edge 是**误读**——它是健康偏强的进化。

**③ mega_orb（orb）终判：非 OP，疑偏弱 → P3b（候选加强非削）**。边际仅 +3（backlog 29→27，几乎不清场），且三者中最不安全（hp_min 0.18，混编仍险死）。**非 OP**。与历史「mega_orb 数值倒退」审计呼应——mega_orb 进化贡献微弱。**本轮不调数值**，记 P3b（方向是**加强/重做**而非削，守宪法 P4「进化=质变」）。

**④ 方法学**：控制组 explosion（+25，已知最强清场）成功给边际定标——三者干净分层 explosion(+25) ≫ knife(+16) ≫ orb(+3)，证明 A/B backlog 边际能区分清场强度。共时重叠窗（mixbase 部分种子早死）使比较公平。
