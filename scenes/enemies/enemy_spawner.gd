...
var _music_state: String = ""           # 当前 BGM 轨道(act1/act2/act3/boss)，切换时才重放
...
func _update_music() -> void:
	var want := _music_track_for(_elapsed_time, _boss_alive)
	if want == _music_state:
		return
	_music_state = want
	_play_music_track(want)

# 在 assets/audio/music/ 找 <track>.ogg/.mp3/.wav；找不到返回 ""。
func _resolve_music(track: String) -> String:
	for ext in [".ogg", ".mp3", ".wav"]:
		var p := "res://assets/audio/music/%s%s" % [track, ext]
		if ResourceLoader.exists(p):
			return p
	return ""

func _play_music_track(track: String) -> void:
	var path := _resolve_music(track)
	if path == "":
		return  # 音乐素材未就位 → 安全跳过(由用户从免版权站下载放入)
